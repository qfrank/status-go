...
func TestSplitPayloadInBatches(t *testing.T) {
	...
	response := splitPayloadInBatches(payload, 100, logger)
	...
	response = splitPayloadInBatches(payload, 1+payloadTagSize+1, logger)
	require.NotNil(t, response)
	...
	response = splitPayloadInBatches(payload, 1+payloadTagSize+2+timestampPayloadSize+1+1, logger)
	require.NotNil(t, response)
	require.Len(t, response, 4)
}
